...
for (i in 1:length(exps)) {
  ...
  exp <- exps[i]
  message("[",format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "] - Processing experiment ", exp)

  filename <- paste0("CMIP6-C3S-METACLIP-Provenance_", exp, ".jsonld")
  output.file <- file.path(output.dir, filename)
  exp.subset <- subset(master, subset = experiment == exp)
  unique_modelRuns <- unique(exp.subset[, c("realization_number", "model_ID")])
      
    ds.subset.list <- lapply(1:nrow(unique_modelRuns), function(j) {
      gcm <- unique_modelRuns[j, ]$model_ID
        ...
        ## Initialize graph
            ...
            ## REALIZATION
            label <- unique_modelRuns[j, ]$realization_number
            ...
            info.gcm <- subset(exp.subset, 
                               model_ID == unique_modelRuns[j, ]$model_ID & realization_number == unique_modelRuns[j, ]$realization_number)[1,]
